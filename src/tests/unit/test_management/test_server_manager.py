...
from swagger_mcp_server.management.server_manager import MCPServerManager, ServerError
...
class TestMCPServerManager:
    ...
    async def test_start_server_daemon_mode_success(self):
        ...
        with patch.object(self.manager, '_validate_server_config'), \
             ...
             patch.object(self.manager.daemon_manager, 'start_daemon_server',
    ...
    async def test_start_server_port_in_use(self):
        ...
        with patch.object(self.manager, '_validate_server_config'), \
             patch.object(self.manager.registry, 'is_port_available', return_value=False):
    ...
    async def test_stop_server_success(self):
        ...
        with patch.object(self.manager.registry, 'get_server', return_value=server_instance), \
             patch.object(self.manager.registry, 'update_server_status'), \
    ...
    async def test_get_server_status_success(self):
        ...
        from swagger_mcp_server.management.process_monitor import ProcessMetrics, HealthStatus, HealthLevel
        ...
        with patch.object(self.manager.registry, 'get_server', return_value=server_instance), \
             patch.object(self.manager.process_monitor, 'get_process_metrics',
                         return_value=process_metrics), \
             patch.object(self.manager.process_monitor, 'check_server_health',
                         return_value=health_status), \
             patch.object(self.manager.process_monitor, 'get_server_metrics',
                         return_value=None), \
             patch.object(self.manager.registry, 'update_server_status'):
    ...
    async def test_get_all_servers_status(self):
        ...
        with patch.object(self.manager.registry, 'cleanup_dead_servers'), \
             patch.object(self.manager.registry, 'get_all_servers', return_value=servers), \
    ...
    async def test_wait_for_server_ready_success(self):
        ...
        with patch('swagger_mcp_server.management.server_manager.socket.socket') as mock_socket:
            mock_sock = Mock()
