...
class SecurityRepository(BaseRepository[SecurityScheme]):
    ...
    async def get_by_http_scheme(
        self, http_scheme: str, api_id: Optional[int] = None
    ) -> List[SecurityScheme]:
        ...
        try:
            ...
            raise RepositoryError(
                f"Failed to get security schemes by HTTP scheme: {str(e)}"
            )

    async def get_by_api_key_location(
        self, location: str, api_id: Optional[int] = None
    ) -> List[SecurityScheme]:
    ...
    async def get_statistics(
        ...
        try:
            ...
            if "apiKey" in types:
                ...
                api_key_locations = {
                    row.api_key_location: row.count
