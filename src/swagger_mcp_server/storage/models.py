...
class APIMetadata(Base, TimestampMixin):
    ...
    security_schemes = relationship("SecurityScheme", back_populates="api", cascade="all, delete-orphan")
...
class Endpoint(Base, TimestampMixin):
    ...
    dependencies = relationship("EndpointDependency", back_populates="endpoint", cascade="all, delete-orphan")
...
class Schema(Base, TimestampMixin):
    ...
    # Dependency tracking
    schema_dependencies = Column(JSON)  # List of referenced schema names
    reference_count = Column(Integer, default=0)  # How many times this schema is referenced

    # Relationships
    api = relationship("APIMetadata", back_populates="schemas")
    dependencies = relationship("EndpointDependency", back_populates="schema", cascade="all, delete-orphan")
...
class SecurityScheme(Base, TimestampMixin):
    ...
    api_id = Column(Integer, ForeignKey('api_metadata.id'), nullable=False)
    name = Column(String(255), nullable=False)
    type = Column(String(50), nullable=False)  # apiKey, http, oauth2, openIdConnect, mutualTLS
    description = Column(Text)
...
class EndpointDependency(Base, TimestampMixin):
    ...
    dependency_type = Column(String(50), nullable=False)  # parameter, request_body, response, etc.
    ...
    # Constraints and indexes
    __table_args__ = (
        UniqueConstraint('endpoint_id', 'schema_id', 'dependency_type', name='uq_endpoint_schema_dep'),
