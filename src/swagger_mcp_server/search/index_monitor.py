...
class IndexPerformanceMonitor:
    ...
    async def _check_optimization_triggers(self, metrics: IndexMetrics) -> None:
        ...
        if metrics.fragmentation_ratio > self.thresholds["optimization_trigger_fragmentation"]:
    ...
    async def _perform_optimization(self, event: IndexOptimizationEvent) -> None:
        ...
        if event.size_before_mb > 0:
            event.size_reduction_percent = (
                (event.size_before_mb - event.size_after_mb) / event.size_before_mb * 100
            )

        if event.query_time_before_ms > 0:
            event.performance_improvement_percent = (
                (event.query_time_before_ms - event.query_time_after_ms) / event.query_time_before_ms * 100
            )
    ...
    async def get_performance_summary(self, time_period: str = "24h") -> Dict[str, Any]:
        ...
        summary = {
            ...
            "recommendations": await self._generate_index_recommendations(relevant_metrics)
        }
    ...
    def _calculate_size_stats(self, metrics: List[IndexMetrics]) -> Dict[str, Any]:
        ...
        return {
            ...
            "document_growth": doc_counts[-1] - doc_counts[0] if len(doc_counts) > 1 else 0,
            "avg_size_per_document_kb": (sizes_mb[-1] * 1024 / max(doc_counts[-1], 1)) if doc_counts else 0
        }

    def _calculate_performance_stats(self, metrics: List[IndexMetrics]) -> Dict[str, Any]:
        ...
        return {
            ...
            "nfr_compliance_percent": len([t for t in query_times if t <= 100]) / len(query_times) * 100 if query_times else 0
        }
    ...
    def _calculate_health_stats(self, metrics: List[IndexMetrics]) -> Dict[str, Any]:
        ...
        health_score = 100
        if current_fragmentation > self.thresholds["optimization_trigger_fragmentation"]:
            health_score -= 30
        if current_cache_rate < self.thresholds["min_cache_hit_rate"]:
            health_score -= 20

        avg_query_time = statistics.mean([m.query_time_ms for m in metrics]) if metrics else 0
    ...
    def _get_optimization_summary(self) -> Dict[str, Any]:
        ...
        recent_optimizations = [
            opt for opt in self.optimization_events
        ...
        return {
            ...
            ]) if successful_opts else 0,
            "avg_size_reduction_percent": statistics.mean([
                opt.size_reduction_percent for opt in successful_opts if opt.size_reduction_percent
            ]) if successful_opts else 0,
            "avg_performance_improvement_percent": statistics.mean([
                opt.performance_improvement_percent for opt in successful_opts if opt.performance_improvement_percent
            ]) if successful_opts else 0
        }

    def _get_performance_alerts(self, metrics: List[IndexMetrics]) -> List[Dict[str, Any]]:
        ...
        if current.query_time_ms > self.thresholds["max_query_time_ms"]:
            alerts.append({
                "level": "warning" if current.query_time_ms < self.thresholds["max_query_time_ms"] * 2 else "critical",
                "type": "performance",
        ...
        # Fragmentation alert
        if current.fragmentation_ratio > self.thresholds["optimization_trigger_fragmentation"]:
            alerts.append({
                "level": "warning",
                "type": "fragmentation",
        ...
        if current.storage_utilization > self.thresholds["max_storage_utilization"]:
            alerts.append({
                "level": "critical" if current.storage_utilization > 0.9 else "warning",
                "type": "storage",
    ...
    async def _generate_index_recommendations(self, metrics: List[IndexMetrics]) -> List[Dict[str, Any]]:
        ...
        if current.cache_hit_rate < self.thresholds["min_cache_hit_rate"]:
            recommendations.append({
                ...
                "actions": ["Increase cache size", "Review cache eviction policy"]
            })

        # Storage capacity recommendation
        if current.storage_utilization > 0.7:
            recommendations.append({
                "priority": "medium" if current.storage_utilization < 0.85 else "high",
                "type": "capacity",
                "title": "Monitor Storage Capacity",
                "description": f"Storage utilization ({current.storage_utilization:.1%}) requires attention",
                "actions": ["Monitor growth trends", "Plan storage expansion", "Implement data archiving"]
            })
        ...
        if avg_query_time > self.thresholds["max_query_time_ms"]:
            recommendations.append({
                ...
                "actions": ["Analyze slow queries", "Optimize index structure", "Review query patterns"]
            })

        return recommendations

    async def project_index_growth(self, projection_days: int = 30) -> IndexGrowthProjection:
        ...
        recent_metrics = list(self.metrics_history)[-min(7, len(self.metrics_history)):]  # Last week
        ...
        time_diff_days = (last_metric.timestamp - first_metric.timestamp).total_seconds() / (24 * 3600)

        if time_diff_days > 0:
            size_growth_rate = (last_metric.index_size_mb - first_metric.index_size_mb) / time_diff_days
            doc_growth_rate = (last_metric.document_count - first_metric.document_count) / time_diff_days
        else:
            size_growth_rate = 0
            doc_growth_rate = 0

        # Project future values
        projected_size_mb = last_metric.index_size_mb + (size_growth_rate * projection_days)
        projected_document_count = last_metric.document_count + int(doc_growth_rate * projection_days)
        ...
        if size_growth_rate > 0 and last_metric.storage_utilization > 0:
            ...
            available_storage_mb = (last_metric.index_size_mb / last_metric.storage_utilization) * (1 - 0.8)
        ...
        if doc_growth_rate > 1000:  # More than 1000 docs per day
            recommendations.append("Monitor indexing performance under high document volume")
