...
class SearchPerformanceMonitor:
    ...
    async def monitor_search_operation(
        ...
        **kwargs
        ...
        try:
            ...
            if hasattr(search_func, '__name__') and 'process_query' in search_func.__name__:
            ...
            analytics = SearchAnalytics(
                ...
                index_query_time=timing_info.get("index_query", total_time * 0.6),  # Estimate
                result_processing_time=timing_info.get("result_processing", total_time * 0.2),  # Estimate
                result_count=self._extract_result_count(result),
                correlation_id=correlation_id,
                performance_grade=self._classify_performance(total_time),
                exceeded_threshold=total_time > self.performance_thresholds["response_time_warning"],
                cache_hit=cache_hit,
    ...
    async def get_performance_summary(self, time_period: str = "1h") -> Dict[str, Any]:
        ...
        relevant_data = [
            a for a in self.analytics_data
        ...
        summary = {
            ...
            "response_time_metrics": self._calculate_response_time_metrics(relevant_data),
            ...
            "optimization_recommendations": await self._generate_optimization_recommendations(relevant_data),
    ...
    def _calculate_response_time_metrics(self, analytics_data: List[SearchAnalytics]) -> Dict[str, float]:
        ...
        return {
            ...
            "p95": response_times[int(count * 0.95)] if count > 20 else response_times[-1],
            "p99": response_times[int(count * 0.99)] if count > 100 else response_times[-1],
            "min": min(response_times),
            "max": max(response_times),
            "std_dev": statistics.stdev(response_times) if count > 1 else 0,
            "nfr1_compliance": len([t for t in response_times if t <= 200]) / count * 100
        }

    def _calculate_query_volume_metrics(self, analytics_data: List[SearchAnalytics]) -> Dict[str, Any]:
        ...
        return {
            ...
            "popular_filters": dict(sorted(filter_usage.items(), key=lambda x: x[1], reverse=True)[:5]),
            "hourly_volume": dict(hourly_volume)
        }

    def _calculate_search_effectiveness(self, analytics_data: List[SearchAnalytics]) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_results_per_query": statistics.mean([a.result_count for a in analytics_data]),
            "median_results_per_query": statistics.median([a.result_count for a in analytics_data]),
            "performance_grade_distribution": self._calculate_performance_grade_distribution(analytics_data),
            "user_satisfaction": self._calculate_user_satisfaction(analytics_data)
        }

    def _calculate_performance_trends(self, analytics_data: List[SearchAnalytics]) -> Dict[str, Any]:
    ...
    def _calculate_cache_performance(self, analytics_data: List[SearchAnalytics]) -> Dict[str, Any]:
        ...
        cached_avg_time = statistics.mean([a.total_response_time for a in cached_queries]) if cached_queries else 0
        non_cached_avg_time = statistics.mean([a.total_response_time for a in non_cached_queries]) if non_cached_queries else 0
    ...
    def _calculate_concurrent_usage_metrics(self, analytics_data: List[SearchAnalytics]) -> Dict[str, Any]:
        ...
        return {
            ...
            "concurrent_distribution": self._calculate_distribution(concurrent_counts, [1, 5, 10, 25, 50])
        }

    async def _generate_optimization_recommendations(self, analytics_data: List[SearchAnalytics]) -> List[Dict[str, Any]]:
        ...
        slow_queries = [a for a in analytics_data if a.total_response_time > self.performance_thresholds["response_time_warning"]]
        ...
        cache_hit_rate = statistics.mean([1 if a.cache_hit else 0 for a in analytics_data])
        ...
        index_times = [a.index_query_time for a in analytics_data if a.index_query_time > 0]
        if index_times and statistics.mean(index_times) > self.performance_thresholds["index_query_time_warning"]:
            recommendations.append({
                "type": "index",
                "priority": "medium",
        ...
        if max(concurrent_counts) > 20:
            high_concurrency_slow = [
                ...
                if a.concurrent_queries > 10 and a.total_response_time > self.performance_thresholds["response_time_warning"]
            ]
            if len(high_concurrency_slow) > len([a for a in analytics_data if a.concurrent_queries > 10]) * 0.3:
                recommendations.append({
                    "type": "scalability",
                    "priority": "high",
    ...
    async def _check_performance_thresholds(self, analytics: SearchAnalytics) -> None:
        ...
        if analytics.total_response_time > self.performance_thresholds["response_time_critical"]:
        ...
        elif analytics.total_response_time > self.performance_thresholds["response_time_warning"]:
            await self._create_alert(
                AlertLevel.WARNING,
                "Slow Response Time",
    ...
    async def _update_real_time_metrics(self, analytics: SearchAnalytics) -> None:
        ...
        if recent_data:
            self.current_metrics["avg_response_time_hour"] = statistics.mean([a.total_response_time for a in recent_data])
            self.current_metrics["cache_hit_rate_hour"] = statistics.mean([1 if a.cache_hit else 0 for a in recent_data])
    ...
    def _calculate_performance_grade_distribution(self, analytics_data: List[SearchAnalytics]) -> Dict[str, int]:
    ...
    def _calculate_user_satisfaction(self, analytics_data: List[SearchAnalytics]) -> Dict[str, Any]:
        """Calculate user satisfaction metrics."""
        satisfaction_scores = [a.user_satisfaction for a in analytics_data if a.user_satisfaction is not None]
        ...
        return {
            ...
            "satisfaction_distribution": self._calculate_distribution(satisfaction_scores, [1, 2, 3, 4, 5])
        }

    def _calculate_distribution(self, values: List[float], buckets: List[float]) -> Dict[str, int]:
    ...
    def _analyze_common_failed_terms(self, failed_searches: List[SearchAnalytics]) -> List[str]:
        ...
        return [term for term, count in sorted(term_counts.items(), key=lambda x: x[1], reverse=True)]

    async def _record_search_failure(self, correlation_id: str, error: str, duration: float) -> None:
