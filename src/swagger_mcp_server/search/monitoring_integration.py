...
class SearchMonitoringIntegration:
    ...
    async def export_search_metrics(self) -> List[SearchMetricExport]:
        ...
        try:
            ...
            performance_summary = await self.search_monitor.get_performance_summary("1h")
            ...
            for percentile, value in response_times.items():
                if isinstance(value, (int, float)):
                    metrics.append(SearchMetricExport(
                        metric_name=f"search_response_time_{percentile}",
                        metric_value=value,
                        metric_type="gauge",
                        timestamp=current_time,
                        tags={"component": "search", "metric_type": "response_time"}
                    ))
            ...
            for metric_name, value in volume_metrics.items():
                if isinstance(value, (int, float)):
                    metrics.append(SearchMetricExport(
                        ...
                        timestamp=current_time,
                        tags={"component": "search", "metric_type": "volume"}
                    ))
            ...
            for metric_name, value in effectiveness.items():
                if isinstance(value, (int, float)):
                    metrics.append(SearchMetricExport(
                        ...
                        metric_type="gauge",
                        timestamp=current_time,
                        tags={"component": "search", "metric_type": "effectiveness"}
                    ))
            ...
            for metric_name, value in cache_performance.items():
                if isinstance(value, (int, float)):
                    metrics.append(SearchMetricExport(
                        ...
                        metric_type="gauge",
                        timestamp=current_time,
                        tags={"component": "search", "metric_type": "cache"}
                    ))
            ...
            for metric_name, value in current_metrics.items():
                if isinstance(value, (int, float)):
                    metrics.append(SearchMetricExport(
                        ...
                        metric_type="gauge",
                        timestamp=current_time,
                        tags={"component": "search", "metric_type": "operational"}
                    ))
    ...
    async def log_search_analytics(self, analytics: SearchAnalytics) -> None:
        ...
        try:
            ...
            if analytics.exceeded_threshold:
                self.logger.warning("Search performance threshold exceeded", extra=log_data)
        ...
        try:
            ...
            if hasattr(self.mcp_monitor, 'get_performance_thresholds'):
                ...
                if "response_time_warning" in mcp_thresholds:
                    self.search_monitor.performance_thresholds["response_time_warning"] = mcp_thresholds["response_time_warning"]

                if "response_time_critical" in mcp_thresholds:
                    self.search_monitor.performance_thresholds["response_time_critical"] = mcp_thresholds["response_time_critical"]

                self.logger.info("Performance thresholds synchronized with MCP monitoring")
    ...
    async def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "mcp_integration_enabled": self.config.enable_mcp_integration and self.mcp_monitor is not None,
            ...
            "export_task_running": self.export_task is not None and not self.export_task.done()
        }

    async def generate_monitoring_report(self, time_period: str = "24h") -> Dict[str, Any]:
        ...
        try:
            ...
            report = {
                ...
                "performance_summary": {
                    "overview": dashboard_data.get("performance", {}).get("overview", {}),
                    "nfr1_compliance": dashboard_data.get("performance", {}).get("nfr1_compliance", {}),
                    "trends": dashboard_data.get("performance", {}).get("trends", {})
                },

                "analytics_summary": {
                    "effectiveness": dashboard_data.get("analytics", {}).get("effectiveness", {}),
                    "top_patterns": dashboard_data.get("analytics", {}).get("query_patterns", [])[:5],
                    "user_behavior": dashboard_data.get("analytics", {}).get("user_behavior", {})
                },

                "system_health": {
                    "overall_health": dashboard_data.get("status", {}).get("overall_health", "unknown"),
                    "system_status": dashboard_data.get("status", {}).get("system_status", "unknown"),
                    "index_health": dashboard_data.get("index", {}).get("health", {}),
                    "active_alerts": len(dashboard_data.get("alerts", {}).get("active_alerts", []))
                },

                "recommendations": {
                    "priority_actions": dashboard_data.get("recommendations", {}).get("priority_actions", []),
                    "performance_recommendations": dashboard_data.get("recommendations", {}).get("performance", [])[:3],
                    "analytics_recommendations": dashboard_data.get("recommendations", {}).get("analytics", [])[:3]
                },

                "exported_metrics": {
                    "total_metrics": len(current_metrics),
                    "metric_categories": list(set(m.tags.get("metric_type", "unknown") for m in current_metrics)),
