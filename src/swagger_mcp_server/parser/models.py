...
class NormalizedParameter(BaseModel):
    ...
    deprecated: bool = Field(default=False, description="Whether parameter is deprecated")
    ...
    items_schema: Optional[Dict[str, Any]] = Field(None, description="Array items schema")
    additional_properties: Optional[Dict[str, Any]] = Field(None, description="Additional properties for objects")

    # Extension properties
    extensions: Dict[str, Any] = Field(default_factory=dict, description="OpenAPI extensions (x-*)")
...
class NormalizedRequestBody(BaseModel):
    ...
    content: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Content by media type")
...
class NormalizedResponse(BaseModel):
    ...
    headers: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Response headers")
    content: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Content by media type")
    links: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Response links")
...
class NormalizedSecurity(BaseModel):
    ...
    location: Optional[ParameterLocation] = Field(None, alias="in", description="API key location")
...
class NormalizedEndpoint(BaseModel):
    ...
    external_docs: Optional[Dict[str, Any]] = Field(None, description="External documentation")

    # Parameters and body
    parameters: List[NormalizedParameter] = Field(default_factory=list, description="Operation parameters")
    request_body: Optional[NormalizedRequestBody] = Field(None, description="Request body")

    # Responses
    responses: Dict[str, NormalizedResponse] = Field(default_factory=dict, description="Responses by status code")

    # Security
    security: List[List[NormalizedSecurityRequirement]] = Field(default_factory=list, description="Security requirements")

    # Callbacks and links
    callbacks: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Operation callbacks")
    ...
    parameter_names: List[str] = Field(default_factory=list, description="All parameter names for search")
    response_schemas: List[str] = Field(default_factory=list, description="Response schema references")

    # Relationships
    schema_dependencies: Set[str] = Field(default_factory=set, description="Referenced schema components")
    security_dependencies: Set[str] = Field(default_factory=set, description="Referenced security schemes")
...
class NormalizedSchema(BaseModel):
    ...
    # Object properties
    properties: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Object properties")
    required: List[str] = Field(default_factory=list, description="Required properties")
    additional_properties: Optional[Union[bool, Dict[str, Any]]] = Field(None, description="Additional properties")
    ...
    unique_items: Optional[bool] = Field(None, description="Whether array items must be unique")
    ...
    multiple_of: Optional[Union[int, float]] = Field(None, description="Multiple of value")
    ...
    not_schema: Optional[Dict[str, Any]] = Field(None, alias="not", description="Not schema")

    # Conditional
    if_schema: Optional[Dict[str, Any]] = Field(None, alias="if", description="If schema")
    then_schema: Optional[Dict[str, Any]] = Field(None, alias="then", description="Then schema")
    else_schema: Optional[Dict[str, Any]] = Field(None, alias="else", description="Else schema")
    ...
    # OpenAPI specific
    discriminator: Optional[Dict[str, Any]] = Field(None, description="Discriminator object")
    xml: Optional[Dict[str, Any]] = Field(None, description="XML metadata")
    external_docs: Optional[Dict[str, Any]] = Field(None, description="External documentation")

    # Relationships and dependencies
    dependencies: Set[str] = Field(default_factory=set, description="Schema dependencies ($ref)")
    used_by: Set[str] = Field(default_factory=set, description="Endpoints using this schema")

    # Search optimization
    searchable_text: str = Field(default="", description="Combined searchable text")
    property_names: List[str] = Field(default_factory=list, description="Property names for search")
...
class NormalizedAPI(BaseModel):
    ...
    # Servers
    servers: List[Dict[str, Any]] = Field(default_factory=list, description="Server information")

    # Core components
    endpoints: List[NormalizedEndpoint] = Field(default_factory=list, description="All API endpoints")
    schemas: Dict[str, NormalizedSchema] = Field(default_factory=dict, description="Schema components")
    security_schemes: Dict[str, NormalizedSecurity] = Field(default_factory=dict, description="Security schemes")

    # Additional components
    responses: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Reusable responses")
    parameters: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Reusable parameters")
    examples: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Reusable examples")
    request_bodies: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Reusable request bodies")
    headers: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Reusable headers")
    links: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Reusable links")
    callbacks: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Reusable callbacks")

    # Global security
    security: List[List[NormalizedSecurityRequirement]] = Field(default_factory=list, description="Global security requirements")
    ...
    # Processing metadata
    normalized_at: datetime = Field(default_factory=datetime.now, description="Normalization timestamp")
    ...
    security_scheme_count: int = Field(default=0, description="Total number of security schemes")
    extension_count: int = Field(default=0, description="Total number of extensions")

    # Relationships
    dependency_graph: Dict[str, Set[str]] = Field(default_factory=dict, description="Component dependency graph")
    reverse_dependencies: Dict[str, Set[str]] = Field(default_factory=dict, description="Reverse dependency mapping")

    # Search optimization
    all_searchable_text: str = Field(default="", description="Combined searchable text for API")
    endpoint_tags: Set[str] = Field(default_factory=set, description="All unique endpoint tags")

    # Extension properties
    extensions: Dict[str, Any] = Field(default_factory=dict, description="Root-level OpenAPI extensions (x-*)")
